...
export function calculateCardMove(
  ...
  if (sourceColumnId === targetColumnId) {
    ...
    const columnCards = targetColumn.cards.sort(
    ...
    const targetCards = targetColumn.cards.sort(
      (a, b) => a.position - b.position,
    );
...
export function applyDragUpdatesOptimistically(
  ...
  for (const update of updates) {
    ...
    let sourceColumn: ColumnWithCards | null = null;
    let cardIndex = -1;
...
export function getDropTarget(
  ...
  if (
    ...
    (overId.startsWith("column-") || overId.startsWith("column-drop-"))
  ) {
    // Dropping over a column (append to end)
    const columnId = parseInt(
      overId.replace("column-", "").replace("column-drop-", ""),
    );
