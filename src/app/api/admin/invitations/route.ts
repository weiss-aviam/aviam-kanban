  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        pending: transformedInvitations.filter((i) => i.status === "pending")
          .length,
        accepted: transformedInvitations.filter((i) => i.status === "accepted")
          .length,
        expired: transformedInvitations.filter((i) => i.status === "expired")
          .length,
      },
    });
  } catch (error) {
