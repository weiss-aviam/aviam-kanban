...
export async function GET(request: NextRequest) {
  try {
    ...
    const { page, limit, action, targetUserId, startDate, endDate } =
      filtersResult.data;
    ...
    interface AuditLogRow {
      ...
      target_users: { id: string; email: string; name: string } | null;
    }

    const logs = (auditLogs as unknown as AuditLogRow[]).map((log) => ({
      id: log.id,
      action: log.action,
