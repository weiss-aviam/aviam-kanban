...
export function useCardActionsWithStore({
  ...
  const { updateCard, deleteCard, addCard, setError, clearError } =
    useAppActions();
  ...
  const handleMoveToColumn = useCallback(
    async (card: Card, columnId: number) => {
      // Use local UI feedback instead of global page loading
      clearError();
  ...
  const handleDeleteCard = useCallback(
    async (card: Card, skipConfirmation = false) => {
      if (!skipConfirmation) {
  ...
  const handleBulkUpdate = useCallback(
    async (updates: { id: string; [key: string]: unknown }[]) => {
      // Use local UI feedback instead of global page loading
      clearError();
