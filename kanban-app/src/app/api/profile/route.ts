...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { error: updateAuthErr } = await supabase.auth.updateUser({ data: { name } });
    if (updateAuthErr) {
      console.error('Auth metadata update failed:', updateAuthErr);
      return NextResponse.json({ error: 'Failed to update profile (auth)' }, { status: 500 });
    }
    ...
    return NextResponse.json({ message: 'Profile updated', user: { id: user.id, name } });
  } catch (error) {
