...
export function useAuditLogs(options: UseAuditLogsOptions) {
   ...
   */
  const fetchAuditLogs = useCallback(async (newFilters: AuditLogFilters = {}) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (currentFilters.action) searchParams.append('action', currentFilters.action);
      if (currentFilters.targetUserId) searchParams.append('targetUserId', currentFilters.targetUserId);
      if (currentFilters.startDate) searchParams.append('startDate', currentFilters.startDate);
      if (currentFilters.endDate) searchParams.append('endDate', currentFilters.endDate);
   ...
   */
  const formatLogDetails = useCallback((action: string, details: any) => {
  ...
  const exportLogs = useCallback(() => {
    ...
    const csvHeaders = ['Date', 'Action', 'Admin User', 'Target User', 'Details', 'IP Address'];
