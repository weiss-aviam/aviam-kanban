...
export function useBoardMemberships(options: UseBoardMembershipsOptions) {
   ...
   */
  const fetchMemberships = useCallback(async (params: {
    page?: number;
    limit?: number;
    search?: string;
    role?: string;
   ...
   */
  const updateMembership = useCallback(async (data: UpdateMembershipData) => {
    setError(null);
   ...
   */
  const bulkUpdateMemberships = useCallback(async (updates: UpdateMembershipData[]) => {
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to bulk update memberships');
      }
      ...
      handleSuccess(`Successfully updated ${updates.length} membership${updates.length > 1 ? 's' : ''}`);
      return result;
    } catch (err) {
      handleError(err);
      throw err;
   ...
   */
  const getMembershipById = useCallback((userId: string) => {
   ...
   */
  const getMembershipsByRole = useCallback((role: 'owner' | 'admin' | 'member' | 'viewer') => {
   ...
   */
  const canChangeRole = useCallback((currentUserRole: string, targetRole: string) => {
