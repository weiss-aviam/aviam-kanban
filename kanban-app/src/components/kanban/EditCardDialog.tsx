...
import { Loader2, Trash2, File as FileIcon, FileText, FileArchive, FileCode, FileSpreadsheet, FileVideo, FileAudio, Paperclip } from 'lucide-react';
import type { Card, Column, User, Label as DatabaseLabel, CardPriority } from '@/types/database';
...
export function EditCardDialog({
  ...
  const CardFormSchema = z.object({
    ...
    assigneeId: z.string().default('none'),
  ...
  type CardFormValues = {
    ...
    columnId: string;
    description: string;
    assigneeId: string; // 'none' or user id
  ...
  const form = useForm<CardFormValues>({
    ...
    defaultValues: {
      title: '',
      description: '',
      columnId: card ? String(card.columnId ?? '') : (defaultColumnId ? String(defaultColumnId) : ''),
      assigneeId: currentUser?.id || 'none',

      priority: (card?.priority as CardPriority) || 'medium',
    },
  });
  const { control, register, handleSubmit: rhfHandleSubmit, watch, reset } = form;

  // Comments state
  const [comments, setComments] = useState<Array<{ id: number; body: string; createdAt: string; author: { id: string; name: string | null; email: string } }>>([]);
  const [commentsLoading, setCommentsLoading] = useState(false);
  const [commentBody, setCommentBody] = useState('');

  // Attachments state
  const [attachments, setAttachments] = useState<Array<{ name: string; path: string; url: string; createdAt?: string; size?: number }>>([]);
  ...
  useEffect(() => {
    ...
    const fetchAttachments = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase.storage.from('card-attachments').list(card.id, { limit: 100, sortBy: { column: 'created_at', order: 'desc' } });
        if (!error && data) {
          const resolveUrl = async (path: string) => {
            const { data, error } = await supabase.storage.from('card-attachments').download(path);
          ...
          const files = await Promise.all(
            data.map(async (f: any) => {
              ...
              return { name: f.name, path, url, createdAt: f.created_at as any, size: f.metadata?.size as any };
            })
          );
          setAttachments((prev) => {
            try { prev.forEach(a => URL.revokeObjectURL(a.url)); } catch {}
            return files;
          });
        } else if (error) {
  ...
  const handleUploadFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      if (oversized.length > 0) {
        setError(`Some files exceed 10 MB and were skipped: ${oversized.map(f => f.name).join(', ')}`);
      }
      for (const file of filesArr) {
        if (file.size > MAX_FILE_SIZE) continue;
        const safeName = `${Date.now()}_${file.name}`;
        const path = `${card.id}/${safeName}`;
        const { error } = await supabase.storage.from('card-attachments').upload(path, file, { upsert: false, cacheControl: '3600' });
        if (error) {
      ...
      // Refresh list
      const { data, error: listErr } = await supabase.storage.from('card-attachments').list(card.id, { limit: 100, sortBy: { column: 'created_at', order: 'desc' } });
      if (listErr) {
        ...
        const filesOut = await Promise.all(
          data.map(async (f: any) => {
            ...
            return { name: f.name, path: p, url, createdAt: f.created_at as any, size: f.metadata?.size as any };
          })
        );
        setAttachments((prev) => {
          try { prev.forEach(a => URL.revokeObjectURL(a.url)); } catch {}
  ...
  const isImageFile = (name: string) => imageExts.has((name.split('.').pop() || '').toLowerCase());

  const AttachmentIcon = ({ name, className = 'h-10 w-10 text-muted-foreground' }: { name: string; className?: string }) => {
    const ext = (name.split('.').pop() || '').toLowerCase();
    if (['txt','md','rtf','doc','docx','pdf'].includes(ext)) return <FileText className={className} />;
    if (['zip','rar','7z','gz','bz2','tar'].includes(ext)) return <FileArchive className={className} />;
    if (['xls','xlsx','csv'].includes(ext)) return <FileSpreadsheet className={className} />;
    if (['js','ts','tsx','jsx','json','yml','yaml','xml','html','css','scss','py','rb','go','rs','java','kt','php','c','cpp','h','sh'].includes(ext)) return <FileCode className={className} />;
    if (['mp4','webm','mov','mkv'].includes(ext)) return <FileVideo className={className} />;
    if (['mp3','wav','flac','ogg','m4a'].includes(ext)) return <FileAudio className={className} />;
  ...
  const handleDeleteAttachment = async (path: string) => {
    ...
    setUploading(true);
    try {
      const { error: removeErr } = await supabase.storage.from('card-attachments').remove([path]);
      if (removeErr) {
      ...
      if (listErr) {
        ...
        const resolveUrl = async (p: string) => {
          const { data, error } = await supabase.storage.from('card-attachments').download(p);
          if (error) throw error;
          return URL.createObjectURL(data);
        };
        const filesOut = await Promise.all(
          data.map(async (f: any) => {
            const p = `${card.id}/${f.name}`;
            const url = await resolveUrl(p);
            return { name: f.name, path: p, url, createdAt: (f as any).created_at as any, size: (f as any).metadata?.size as any };
          })
        );
        setAttachments((prev) => {
          try { prev.forEach(a => URL.revokeObjectURL(a.url)); } catch {}
  ...
  const selectedColumn = columns.find(col => col.id === parseInt(watchedColumnId || '0'));
      ...
      <DialogContent className="sm:max-w-7xl w-[95vw] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={rhfHandleSubmit(handleSubmit)} className="space-y-6">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <MarkdownEditor
                      ...
                      height={200}
                      preview="live"
                      className={isLoading || isDeleting ? 'opacity-50 pointer-events-none' : ''}
                    />
                  )}
                />
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 flex flex-col max-w-[200px]">
                ...
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange} disabled={isLoading || isDeleting}>
          ...
          {card ? (
            <div className="mt-4 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  {comments.map((c) => (
                    <div key={c.id} className="rounded-md border p-3">
                      <div className="text-sm font-medium">{c.author.name || c.author.email}</div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    />
                    <Button type="button" onClick={handleSubmitComment} disabled={!commentBody.trim() || commentsLoading || isLoading || isDeleting}>
                      {commentsLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                      Comment
                    </Button>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleFileInputClick(); } }}
                ...
                {attachments.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {attachments.map((a) => (
                      <li key={a.path} className="flex items-center justify-between rounded border p-2">
                        ...
                        <Button type="button" variant="ghost" size="sm" onClick={() => handleDeleteAttachment(a.path)} disabled={uploading || isLoading || isDeleting}>
            ...
            <div className="mt-4 text-sm text-muted-foreground">Create the card to enable comments and attachments.</div>
          )}
