...
import { History, Filter, ChevronLeft, ChevronRight, Calendar, User, Activity } from 'lucide-react';
...
export function AuditLogTable({ boardId, refreshTrigger }: AuditLogTableProps) {
  ...
  const getActionBadge = (action: string) => {
    const actionConfig = {
      invite_user: { label: 'User Invited', color: 'bg-blue-50 text-blue-700 border-blue-200' },
      update_user: { label: 'User Updated', color: 'bg-yellow-50 text-yellow-700 border-yellow-200' },
      remove_user: { label: 'User Removed', color: 'bg-red-50 text-red-700 border-red-200' },
      reset_password: { label: 'Password Reset', color: 'bg-purple-50 text-purple-700 border-purple-200' },
      update_role: { label: 'Role Changed', color: 'bg-green-50 text-green-700 border-green-200' },
      bulk_update_roles: { label: 'Bulk Role Update', color: 'bg-orange-50 text-orange-700 border-orange-200' },
    };
    ...
    switch (action) {
      ...
      case 'update_user':
        ...
        if (details.nameChanged) {
          changes.push(`name: "${details.nameChanged.from}" → "${details.nameChanged.to}"`);
        }
        if (details.roleChanged) {
          changes.push(`role: ${details.roleChanged.from} → ${details.roleChanged.to}`);
        }
        return changes.join(', ');
      case 'remove_user':
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      <p className="font-medium">
                        {log.adminUser?.name || log.adminUser?.email || 'Unknown admin'}
                      </p>
                      <p className="text-sm text-gray-600">
