...
import { MarkdownViewer, CompactMarkdownViewer } from '@/components/ui/markdown-viewer';
...
describe('Priority System', () => {
  describe('PrioritySelector', () => {
    ...
    it('calls onChange when priority is selected', async () => {
      ...
      render(
        <PrioritySelector value="medium" onChange={mockOnChange} />
      );
  ...
  describe('PriorityBadge', () => {
    ...
    it('renders different sizes correctly', () => {
      const { rerender } = render(<PriorityBadge priority="medium" size="sm" />);
...
describe('Markdown System', () => {
  describe('MarkdownViewer', () => {
    it('renders markdown content correctly', () => {
      render(
        <MarkdownViewer content="# Heading\n\nThis is **bold** text." />
      );
  ...
  describe('CompactMarkdownViewer', () => {
    it('renders in compact mode', () => {
      ...
      expect(screen.getByText('Long heading that should be truncated')).toBeInTheDocument();
    });

    it('respects maxLines prop', () => {
      const longContent = 'Line 1\n\nLine 2\n\nLine 3\n\nLine 4';
      render(
        <CompactMarkdownViewer content={longContent} maxLines={2} />
      );
...
describe('Card Actions Hook', () => {
  ...
  it('handles errors gracefully', async () => {
    ...
    await expect(result.current.handlePriorityChange(mockCard, 'low')).rejects.toThrow('Test error');
...
describe('Board Filters Hook', () => {
  it('filters cards by priority correctly', () => {
    ...
    expect(result.current.filteredAndSortedCards.every((card: any) => card.priority === 'high')).toBe(true);
  });
