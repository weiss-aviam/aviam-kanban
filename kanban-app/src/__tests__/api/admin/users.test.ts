...
describe('/api/admin/users', () => {
  ...
  describe('GET /api/admin/users', () => {
    ...
    it('should return forbidden when user is not admin', async () => {
      ...
      (requireAdminAccess as any).mockRejectedValue(new Error('Admin access required'));
    ...
    it('should return paginated users when request is valid', async () => {
      const { createClient } = await import('@/lib/supabase/server');
      const { requireAdminAccess, createAdminClient } = await import('@/lib/supabase/admin');
      ...
      (requireAdminAccess as any).mockResolvedValue({
        userId: mockUser.id,
        boardId: mockBoardId,
        role: 'admin',
      ...
      const mockAdminClient = createAdminClient as any;
  ...
  describe('POST /api/admin/users', () => {
    ...
    it('should successfully invite a user', async () => {
      const { createClient } = await import('@/lib/supabase/server');
      const { requireAdminAccess, createAdminClient, logAdminAction } = await import('@/lib/supabase/admin');
      ...
      (logAdminAction as any).mockResolvedValue(undefined);
