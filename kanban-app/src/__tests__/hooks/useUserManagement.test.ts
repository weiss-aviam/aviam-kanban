...
describe('useUserManagement', () => {
  ...
  describe('fetchUsers', () => {
    it('should fetch users successfully', async () => {
      const mockResponse = {
        users: [
          { id: '1', email: 'user1@example.com', name: 'User 1', role: 'member' },
          { id: '2', email: 'user2@example.com', name: 'User 2', role: 'viewer' },
        ],
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/admin/users?boardId=board-123&page=1&limit=20&sortBy=name&sortOrder=asc')
      );
    ...
    it('should include search and filter parameters', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('search=john&role=admin&sortBy=email&sortOrder=desc')
      );
    });
  });
  ...
  describe('inviteUser', () => {
    it('should invite user successfully', async () => {
      ...
      expect(mockOnSuccess).toHaveBeenCalledWith('User invitation sent successfully');
      expect(result.current.loading).toBe(false);
      expect(result.current.error).toBe(null);
    });
  ...
  describe('updateUser', () => {
    it('should update user successfully', async () => {
      ...
      let updateResult;
      await act(async () => {
        updateResult = await result.current.updateUser('user-123', 'board-123', {
          name: 'Updated Name',
          role: 'admin',
        });
      });

      expect(mockFetch).toHaveBeenCalledWith('/api/admin/users/user-123?boardId=board-123', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: 'Updated Name',
  ...
  describe('removeUser', () => {
    it('should remove user successfully', async () => {
      ...
      expect(mockOnSuccess).toHaveBeenCalledWith('User removed from board successfully');
    });
  });
  ...
  describe('resetPassword', () => {
    it('should reset password successfully', async () => {
      ...
      let resetResult;
      await act(async () => {
        resetResult = await result.current.resetPassword('user-123', 'board-123');
      });

      expect(mockFetch).toHaveBeenCalledWith('/api/admin/users/user-123/reset-password?boardId=board-123', {
        method: 'POST',
      });
  ...
  describe('bulkInviteUsers', () => {
    it('should bulk invite users successfully', async () => {
      ...
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ message: 'User invitation sent successfully' }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ message: 'User invitation sent successfully' }),
      ...
      expect(mockOnSuccess).toHaveBeenCalledWith('Successfully sent 2 invitations');
    });
    ...
    it('should handle partial failures in bulk invite', async () => {
      ...
      expect(mockOnSuccess).toHaveBeenCalledWith('Successfully sent 1 invitation');
