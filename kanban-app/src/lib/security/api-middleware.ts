...
export function withAdminSecurity(
  ...
  return async function secureHandler(
    ...
    try {
      ...
      if (options.validateSchema && ['POST', 'PATCH', 'PUT'].includes(request.method)) {
        try {
...
export const ApiErrors = {
  ...
  ADMIN_ACCESS_REQUIRED: new ApiError('Admin access required', 403, 'ADMIN_ACCESS_REQUIRED'),
  OWNER_ACCESS_REQUIRED: new ApiError('Owner access required', 403, 'OWNER_ACCESS_REQUIRED'),
  BOARD_ACCESS_DENIED: new ApiError('Board access denied', 403, 'BOARD_ACCESS_DENIED'),
  ...
  CANNOT_MODIFY_SELF: new ApiError('Cannot modify your own role', 400, 'CANNOT_MODIFY_SELF'),
  CANNOT_REMOVE_OWNER: new ApiError('Cannot remove board owner', 400, 'CANNOT_REMOVE_OWNER'),
  INVALID_ROLE_ASSIGNMENT: new ApiError('Invalid role assignment', 400, 'INVALID_ROLE_ASSIGNMENT'),
};
...
export function parseQuery<T>(
  ...
  try {
    ...
    throw new ApiError('Query parameter validation failed', 400, 'VALIDATION_ERROR');
  }
}
